...
"$BT/apksigner" sign \
    ...
    --min-sdk-version 28 \
    --out "$OUT/gaime-hello.apk" "$OUT/aligned.apk"

"$BT/apksigner" verify --print-certs "$OUT/gaime-hello.apk" | head -3
ls -la "$OUT/gaime-hello.apk"
echo
echo "install with:"
echo "  adb install -r -g $OUT/gaime-hello.apk"
echo "  adb shell appops set nu.hyperworks.gaimehello SYSTEM_ALERT_WINDOW allow"
echo "  adb shell am start -n nu.hyperworks.gaimehello/.HelloActivity"
